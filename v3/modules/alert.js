...
function checkWaterQualityAlerts(row) {
    ...
    // config.js의 기준을 사용하여 경고 생성
    // I등급 기준을 "정상"으로 사용
    const iGrade = grades['I등급'] || {};
    const iiGrade = grades['II등급'] || {};
    const ivGrade = grades['IV등급'] || {};
    
    // pH 경고 (I등급 기준 사용)
    if (iGrade.pH) {
        if (pH < iGrade.pH.min || pH > iGrade.pH.max) {
            const isCritical = ivGrade.pH && (pH < ivGrade.pH.min || pH > ivGrade.pH.max);
            alerts.push({
                type: 'water_quality',
                level: isCritical ? 'critical' : 'warning',
                message: `⚠️ pH가 ${pH}입니다. (I등급 기준: ${iGrade.pH.min}~${iGrade.pH.max})`,
                manual: { 
                    title: isCritical ? '수질 사고 대응 메뉴얼' : '수질 관리 가이드', 
                    type: isCritical ? 'water_quality_critical' : 'water_quality_warning' 
                }
            });
        }
    }
    ...
    // BOD 경고 (I등급 기준 사용)
    if (iGrade.BOD && iGrade.BOD.max !== undefined) {
        if (BOD > iGrade.BOD.max) {
            const isCritical = ivGrade.BOD && BOD > ivGrade.BOD.max;
            alerts.push({
                type: 'water_quality',
                level: isCritical ? 'critical' : 'warning',
                message: `⚠️ BOD가 ${BOD}입니다. (I등급 기준: ${iGrade.BOD.max} 이하)`,
                manual: { 
                    title: isCritical ? '수질 오염 대응 메뉴얼' : '수질 관리 가이드', 
                    type: isCritical ? 'water_quality_critical' : 'water_quality_warning' 
                }
            });
        }
    ...
    // T-N 경고 (I등급 기준 사용)
    if (iGrade['T-N'] && iGrade['T-N'].max !== undefined) {
        if (TN > iGrade['T-N'].max) {
            const isCritical = ivGrade['T-N'] && TN > ivGrade['T-N'].max;
            alerts.push({
                type: 'water_quality',
                level: isCritical ? 'critical' : 'warning',
                message: `⚠️ 총질소(T-N)가 ${TN}입니다. (I등급 기준: ${iGrade['T-N'].max} 이하)`,
    ...
    // T-P 경고 (I등급 기준 사용)
    if (iGrade['T-P'] && iGrade['T-P'].max !== undefined) {
        if (TP > iGrade['T-P'].max) {
            const isCritical = ivGrade['T-P'] && TP > ivGrade['T-P'].max;
            alerts.push({
                type: 'water_quality',
                level: isCritical ? 'critical' : 'warning',
                message: `⚠️ 총인(T-P)이 ${TP}입니다. (I등급 기준: ${iGrade['T-P'].max} 이하)`,
                manual: { title: '영양염류 관리 가이드', type: 'nutrient_warning' }
            });
        }
    }
